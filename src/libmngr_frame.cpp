...
void libmngrFrame::ToggleMode(bool symbolmode)
{
	...
	if (SymbolMode)
		m_panelView->SetBackgroundColour(wxColour(250, 250, 240));
	else
...
void libmngrFrame::OnPinRightClick(wxGridEvent& event)
{
	...
	case 4:	/* position (section) */
		for (int i = 0; i < PinInfo::SectionCount; i++) {
			menuPopup->Append(IDM_PINSECTION + i, GetPinSectionName(0, i));
...
void libmngrFrame::OnPastePinList(wxCommandEvent& /*event*/)
{
	...
	bool pinfirst = false;
	bool pinlast = false;
	wxArrayString tokens = wxStringTokenize(line, separator, wxTOKEN_RET_EMPTY_ALL);
	if (tokens.Count() < 2)
		return;
	long pinnr;
