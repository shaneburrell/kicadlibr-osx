...
AppFrame::AppFrame( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxFrame( parent, id, title, pos, size, style )
{
	...
	m_mnuRenameLibrary = new wxMenuItem( m_mnuFile, IDM_RENAMELIBRAY, wxString( _("R&ename library...") ) , _("Rename the current library"), wxITEM_NORMAL );
	...
	m_mnuFilter = new wxMenuItem( m_mnuView, IDM_FILTER, wxString( _("Filter...") ) + wxT('\t') + wxT("Ctrl+F"), _("Show only footprints/symbols that match the keywords"), wxITEM_CHECK );
	...
	wxMenuItem* m_mnuUI;
	m_mnuUI = new wxMenuItem( m_mnuPreferences, IDM_DLGOPTIONS, wxString( _("&Application settings...") ) , _("User interface settings"), wxITEM_NORMAL );
	...
	m_btnMove = new wxButton( m_panelTop, IDM_MOVEMODULE, _("&Move"), wxDefaultPosition, wxDefaultSize, 0 );
	m_btnMove->SetToolTip( _("Move the footprint / symbol to the other library.") );
	
	bSizerMid->Add( m_btnMove, 0, wxALL, 5 );
	
	m_btnCopy = new wxButton( m_panelTop, IDM_COPYMODULE, _("&Copy"), wxDefaultPosition, wxDefaultSize, 0 );
	m_btnCopy->SetToolTip( _("Copy the footprint / symbol to the other library.") );
	
	bSizerMid->Add( m_btnCopy, 0, wxALL, 5 );
	
	m_btnDelete = new wxButton( m_panelTop, IDM_DELETEMODULE, _("&Delete"), wxDefaultPosition, wxDefaultSize, 0 );
	m_btnDelete->SetToolTip( _("Remove the selected footprint / symbol.") );
	
	bSizerMid->Add( m_btnDelete, 0, wxALL, 5 );
	
	m_btnRename = new wxButton( m_panelTop, IDM_RENAMEMODULE, _("&Rename"), wxDefaultPosition, wxDefaultSize, 0 );
	m_btnRename->SetToolTip( _("Rename the selected footprint / symbol.") );
	
	bSizerMid->Add( m_btnRename, 0, wxALL, 5 );
	
	m_btnDuplicate = new wxButton( m_panelTop, IDM_RENAMEMODULE, _("D&uplicate"), wxDefaultPosition, wxDefaultSize, 0 );
	...
	m_toolZoomIn = m_toolBar->AddTool( IDT_ZOOMIN, _("Zoom in"), wxBitmap( ZoomIn_xpm ), wxNullBitmap, wxITEM_NORMAL, _("Zoom in (Ctrl +)."), wxEmptyString, NULL ); 
	
	m_toolZoomOut = m_toolBar->AddTool( IDT_ZOOMOUT, _("Zoom out"), wxBitmap( ZoomOut_xpm ), wxNullBitmap, wxITEM_NORMAL, _("Zoom out (Ctrl -)."), wxEmptyString, NULL ); 
	
	m_toolMeasure = m_toolBar->AddTool( IDT_MEASUREMENTS, _("Measure"), wxBitmap( Measure_xpm ), wxNullBitmap, wxITEM_CHECK, _("Show pitch and pad measurements."), wxEmptyString, NULL ); 
	
	m_toolDetailsPanel = m_toolBar->AddTool( IDT_DETAILSPANEL, _("Details panel"), wxBitmap( Information_xpm ), wxNullBitmap, wxITEM_CHECK, _("Show/hide the side panel with parameters."), wxEmptyString, NULL ); 
	
	m_toolLeftFootprint = m_toolBar->AddTool( IDT_LEFTFOOTPRINT, _("Left"), wxBitmap( ArrowLeft_xpm ), wxNullBitmap, wxITEM_CHECK, _("Show left footprint."), wxEmptyString, NULL ); 
	
	m_toolRightFootprint = m_toolBar->AddTool( IDT_RIGHTFOOTPRINT, _("Right"), wxBitmap( ArrowRight_xpm ), wxNullBitmap, wxITEM_CHECK, _("Show right footprint."), wxEmptyString, NULL ); 
	...
	m_gridPinNames->SetColLabelValue( 4, _("side") );
	...
	wxString m_choicePadShapeChoices[] = { _("(varies)"), _("Rectangular"), _("Round"), _("Round + square"), _("Obround"), _("Trapezoid") };
	...
	this->Connect( m_mnuRenameLibrary->GetId(), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( AppFrame::OnRenameLibrary ) );
...
AppFrame::~AppFrame()
{
	...
	this->Disconnect( IDM_RENAMELIBRAY, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( AppFrame::OnRenameLibrary ) );
...
DlgOptions::DlgOptions( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxDialog( parent, id, title, pos, size, style )
{
	...
	m_chkDrawCentreCross = new wxCheckBox( this, wxID_ANY, _("Draw centre cross (footprints)"), wxDefaultPosition, wxDefaultSize, 0 );
	m_chkDrawCentreCross->SetToolTip( _("Draw a cross in the centre position, to mark the \"pick-up\" position for the package.") );
	...
	bSizerOptions->Add( m_chkDisableTemplates, 0, wxRIGHT|wxLEFT, 5 );
	...
	m_chkConfirmOverwrite = new wxCheckBox( this, wxID_ANY, _("Confirm overwrite"), wxDefaultPosition, wxDefaultSize, 0 );
	m_chkConfirmOverwrite->SetToolTip( _("Whether to show a confirmation box when copying/moving a symbol or footprint would overwrite an existing footprint.") );
	
	bSizerOptions->Add( m_chkConfirmOverwrite, 0, wxALL, 5 );
	
	m_chkConfirmDelete = new wxCheckBox( this, wxID_ANY, _("Confirm deletion"), wxDefaultPosition, wxDefaultSize, 0 );
	m_chkConfirmDelete->SetToolTip( _("Whether to show a confirmation box when deleting a symbol or footprint.") );
